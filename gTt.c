...
int countOrdersInFile(const char *filename) {
    ...
    int count = 0;
    char orderNumber[MAX_ORDER_NUM];
    while (fscanf(file, "%19s", orderNumber) == 1) {
        count++;
    }
...
void processOrders(const char* filename) {
    ...
    if (suffix_position != NULL) {
        ...
        // 找不到后缀，处理错误或直接返回
        printf("The file name isn't include '.txt'！\n");
        return;
    }
...
void writeFinal(const char* filename){
	...
	// 确保至少一个数是浮点数，并且Utilization格式为??.? %
        float utilization1 = (total_days_X > 0) ? ((float)total_quantity_X / (total_days_X*300)) * 100 : 0.0f;
	...
	fprintf(file,"\t\tNumber of products produced:       %6d (in total)\n",total_quantity_Y);
	// 确保至少一个数是浮点数，并且Utilization格式为??.? %
        float utilization2 = (total_days_Y > 0) ? ((float)total_quantity_Y / (total_days_Y*400)) * 100 : 0.0f;
        fprintf(file, "\t\tUtilization of the plant:                     %5.1f %%\n\n", utilization2);
	...
	fprintf(file,"\t\tNumber of products produced:       %6d (in total)\n", total_quantity_Z);
	// 确保至少一个数是浮点数，并且Utilization格式为??.? %
        float utilization3 = (total_days_Z > 0) ? ((float)total_quantity_Z / (total_quantity_Z*500)) * 100 : 0.0f;
        fprintf(file, "\t\tUtilization of the plant:                     %5.1f %%\n\n", utilization3);
	
	float utilization4 = (total_days > 0) ? ((float)total_quantity / (total_quantity*1200)) * 100 : 0.0f;
...
int main() {
    ...
    processOrders("Plant_X.txt");
    processOrders("Plant_Y.txt");
    processOrders("Plant_Z.txt");
